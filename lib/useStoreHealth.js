...
export function useStoreHealth() {
  ...
  useEffect(() => {
    ...
    async function checkHealth() {
      ...
      const primaryUrl = baseUrl.endsWith('/api/v1') 
        ? `${baseUrl}/health` 
        : `${baseUrl}/api/v1/health`;

      async function attemptPing(url, timeoutMs) {
      ...
      try {
        let data = null;
        try {
          data = await attemptPing(primaryUrl, 5000);
        } catch (err) {
          // Fallback ke relative path /api/v1/health jika domain utama
          if (primaryUrl !== '/api/v1/health') {
            data = await attemptPing('/api/v1/health', 5000);
          } else {
            throw err;
          }
        }

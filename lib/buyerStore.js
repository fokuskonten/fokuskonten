      ...
      for (const item of o.items) {
        ...
        if (!existing) {
          map.set(itemSku, {
            ...
            coverImage: item.coverImage || `https://cdn.jsdelivr.net/gh/mcjobs-id/fokuskonten-assets@main/toko-digital/${itemSku}/${itemSku}_cover.webp`,
            driveLink: itemDrive,
            lastPurchasedAt: o.createdAt,
            orderId: o.orderId,
            price: item.price || 0,
            licenseKey: item.licenseKey || o.licenseKey || o.license_key || null
          })
        } else {
          if ((!existing.driveLink || existing.driveLink === '#') && itemDrive && itemDrive !== '#') {
            existing.driveLink = itemDrive
          }
          if (item.licenseKey && !existing.licenseKey) {
            existing.licenseKey = item.licenseKey
          }
      ...
      if (cleanSku && cleanSku !== 'MULTI') {
        ...
        if (!existing) {
          map.set(cleanSku, {
            ...
            coverImage: o.coverImage || `https://cdn.jsdelivr.net/gh/mcjobs-id/fokuskonten-assets@main/toko-digital/${cleanSku}/${cleanSku}_cover.webp`,
            driveLink: dl,
            lastPurchasedAt: o.createdAt,
            orderId: o.orderId,
            price: o.price || 0,
            licenseKey: o.licenseKey || o.license_key || null
          })
        } else {
          if ((!existing.driveLink || existing.driveLink === '#') && dl && dl !== '#') {
            existing.driveLink = dl
          }
          if ((o.licenseKey || o.license_key) && !existing.licenseKey) {
            existing.licenseKey = o.licenseKey || o.license_key
          }
          if (o.createdAt > existing.lastPurchasedAt) {
            existing.lastPurchasedAt = o.createdAt
            existing.orderId = o.orderId
    ...
    if (data.success && Array.isArray(data.orders)) {
      data.orders.forEach(srvOrd => {
        addBuyerOrder({
          ...
          licenseKey: srvOrd.license_key || null,

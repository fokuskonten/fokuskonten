...
export async function generateMetadata({ params }) {
  return {
    title: `Author: ${params.slug}`,
    description: `Artikel oleh ${params.slug} di blog FokusKonten.`,
    alternates: {
      canonical: `https://fokuskonten.my.id/blog/author/${params.slug}`,
    },
  }
}

export default function AuthorPage({ params }) {
  const authorPosts = postsData.filter(post => post.author === params.slug)
            ...
            <div className="max-w-3xl mx-auto space-y-4">
              {authorPosts.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-charcoal-500 text-sm">Tidak ada artikel dari author ini</p>
                </div>
              ) : (

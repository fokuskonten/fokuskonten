            ...
            <div className="px-4 py-3 border-t border-neutral-100 space-y-2.5">
              ...
              {isOffline ? (
                <div className="space-y-1.5">
                  <a
                    href={`https://wa.me/6285183011318?text=${encodeURIComponent(`Halo Admin FokusKonten,\n\nSaya ingin memesan ${summary.totalItems} item produk di keranjang:\n${items.map(it => `• [${it.sku}] ${it.title} (${formatRupiah(it.price)})`).join('\n')}\n\nTotal: ${formatRupiah(summary.subtotal)}\nMohon petunjuk pembayaran resmi & pengiriman akses Google Drive.`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                  >
                    ...
                    <span>Pesan via WhatsApp ({summary.totalItems})</span>
                  </a>
                  <p className="text-[10px] text-neutral-400 text-center font-sans">
                    Pemeliharaan sistem berkala &bull; Dilayani langsung via Helpdesk
                  </p>
                </div>
              ) : (
                <button

    ...
    <>
      <div className="bg-white rounded-2xl border border-neutral-200/80 p-6 sm:p-7 shadow-[0_4px_20px_rgba(0,0,0,0.06)] space-y-6">
        ...
        {isOwned && (
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs space-y-2">
            ...
            <p className="text-[11px] text-emerald-700">
              Master desain ini sudah tersimpan di brankas unduhan akun Anda dan siap dibuka di Google Drive.
            </p>
            <div className="pt-1 flex gap-2">
              <Link
                href="/akun/"
                className="px-3 py-1.5 rounded-lg bg-emerald-700 text-white font-bold text-xs hover:bg-emerald-800 transition-colors"
              >
                Buka Brankas Akun Saya →
              </Link>
            </div>
          </div>
        )}
        ...
        <div className="pt-2 space-y-2.5">
          {/* 1. Primary Instant Checkout Button (Midtrans / QRIS) */}
          <button
            type="button"
            onClick={() => setIsCheckoutOpen(true)}
            className="w-full py-3.5 px-4 rounded-xl bg-black hover:bg-neutral-800 active:scale-[0.99] text-white font-display font-extrabold text-sm sm:text-base shadow-lg shadow-black/15 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            <span>⚡</span>
            <span>Beli Langsung (QRIS / Midtrans)</span>
          </button>

          {/* 2. Secondary WhatsApp Consultant Button */}
          <a
            href={waUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full py-2.5 px-4 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-800 font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 text-center cursor-pointer"
          >
